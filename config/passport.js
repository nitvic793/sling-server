...
const JWT_STRATEGY_CONFIG = {
  secretOrKey: '734cf1214c5f33d9628cec598e1e439cd72a22d878e4f66a873a49166c375009',
...
const _onLocalStrategyAuth = (req, email, password, next) => {
  User
    .findOne({email: email}).populate('wards').populate('classes').populate('school')
    .then(user => {
...
const _onJwtStrategyAuth = (req, payload, next) => {
  User
    .findOne({id: payload.id}).populate('wards').populate('classes').populate('school')
    .then(user => {
      if (!user) return next(null, null, sails.config.errors.USER_NOT_FOUND);
      return next(null, user, {});
    })
